...
const userApiHelpers = require("../userApiHelpers");
...
describe("Backend Initialization ", function () {
  ...
  it("should login global admin", async () => {
    const rr = await globalAdminApiHelpers.loginGlobalAdmin();
    expect(rr.res).to.haveOwnProperty("user");
    expect(rr.res).to.haveOwnProperty("token");
  });
  it("should register and login a user", async () => {
    // register  user
    const rr = await userApiHelpers.createUser();
