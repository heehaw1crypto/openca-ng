...
const addResJsonValidation = require("./lib/validateRes");
...
module.exports = function ExpressOpenApi(_routePrefix, _doc, _opts) {
  ...
  let opts = _opts || { namespace: { name: "NO_NAMESPACE" } };
  if (typeof _routePrefix !== "string") {
    routePrefix = defaultRoutePrefix;
    doc = _routePrefix || minimumViableDocument;
    opts = _doc || { namespace: { name: "NO_NAMESPACE" } };
  }
  ...
  const middleware = function OpenApiMiddleware(req, res, next) {
    if (isFirstRequest) {
      middleware.document = generateDocument(
        middleware.document,
  ...
  middleware.path = function (schema = {}) {
    ...
    schemaMiddleware.__SCHEMA_MAP_KEY__ =
      schemaMiddleware.__SCHEMA_MAP_KEY__ || uuidv4();

    setSchema(schemaMiddleware, schema, opts?.namespace);
    ...
    function validSchemaMiddleware(req, res, next) {
      if (!validate) {
        validate = makeValidator(
          middleware,
          getSchema(validSchemaMiddleware, opts?.namespace),
          opts
        );
      }
      // highjack res.json and validate `responses` in the `schema` var
      addResJsonValidation(res, schema);

      return validate(req, res, next);
    }
    validSchemaMiddleware.__SCHEMA_MAP_KEY__ =
      validSchemaMiddleware.__SCHEMA_MAP_KEY__ || uuidv4();

    setSchema(validSchemaMiddleware, schema, opts?.namespace);
  ...
  middleware.component = function (type, name, description) {
    ...
    if (!name && !description) {
      return (
        middleware.document.components && middleware.document.components[type]
      );
    }
    ...
    if (name && !description) {
      if (
        !middleware.document.components ||
        !middleware.document.components[type] ||
        !middleware.document.components[type][name]
      ) {
  ...
  middleware.securitySchemes = middleware.component.bind(
    null,
    "securitySchemes"
  );
